...
<body>
            ...
            <tbody>
                ...
                @if (!is_null($categoria))
                    <tr>
                        ...
                        <td style="padding: 5px; font-size: 10px;">{{ $categoria->nombre }}</td>
                    </tr>
                @endif

                @if (!is_null($proveedor))
                    <tr>
                        <td style="padding: 5px; font-size: 10px;width: 15%;" class="fw-bold">Proveedor:</td>
                        <td style="padding: 5px; font-size: 10px;">
                            {{ $proveedor->nombres ?? '' }}
                            @if (!empty($proveedor->apellidos) && $proveedor->apellidos !== 'null')
                                {{ $proveedor->apellidos }}
                            @endif
                        </td>
                    </tr>
    ...
    <p style="text-align: left; font-size: 12px; margin-bottom: 5px;">
        Total de artículos en baja existencia: <strong>{{ $articulos->count() }}</strong>
    </p>
        ...
        <table
            ...
            <thead>
                <tr style="background-color: #F8285A; color: white; border: none;text-align: center;">
                    ...
                    <th style="border: none; padding: 10px;">Categoria</th>
                    <th style="border: none; padding: 10px;text-align: center;">Existencia</th>
                    <th style="border: none; padding: 10px;text-align: center;">Punto de Pedido</th>
            ...
            <tbody>
                @forelse ($articulos as $articulo)
                    <tr>
                        ...
                        <td
                            ...
                            <div class="d-flex align-items-center">
                                <div class="d-flex justify-content-start flex-column">
                                    ...
                                    <!-- Estado del artículo -->
                                    <span style="font-size: 0.60rem !important;"
                                        class="badge fw-bold badge-status 
                                        {{ $articulo->state_stock === 1 ? 'badge-primary' : ($articulo->state_stock === 2 ? 'badge-warning' : 'badge-danger') }}">
                                        {{ $articulo->state_stock === 1 ? 'Disponible' : ($articulo->state_stock === 2 ? 'Por agotar' : 'Agotado') }}
                                    </span>
                                </div>
                            </div>
                        </td>
                        ...
                        vertical-align: middle;text-align: center;"
                            class="text-gray-900 text-hover-primary mb-1 fs-6">
                            {{ $articulo->punto_pedido }} {{ $articulo->unidad_punto_pedido->nombre }}
                        </td>
